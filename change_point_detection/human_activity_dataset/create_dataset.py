...
def read_data(in_dir, out_file, seg_id, activity, person, segment_id,
              segment_length, segment_start):
    file = open("%s/a%s/p%s/s%s.txt" % (in_dir, str(activity).zfill(2),
                                        str(person), str(segment_id).zfill(2)))
    lines = file.readlines()
    for i in range(segment_start, segment_start + segment_length):
        is_change_point = "0"
        if seg_id != 0 and i == segment_start:
            is_change_point = "1"
...
def create_dataset(in_dir, out_dir):
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    if not os.path.exists("%s/dataset/" % out_dir):
        os.makedirs("%s/dataset/" % out_dir)
