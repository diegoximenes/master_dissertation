...
def majority_voting(tp, email_classificationDt_list):
	delta = 5
	...
	email_classificationId_list = []
	for p in email_classificationDt_list:
		email, class_dt = p[0], p[1]
		class_id = []
		for dt in class_dt: class_id.append(dt_id[dt])
		email_classificationId_list.append([email, class_id])
	
	l = []
	for p in email_classificationId_list:
		for id in p[1]:
			l.append([id, p[0]])
	l.sort()
	...
	classification_out = []
	n_users = len(email_classificationId_list)
	while len(l) > 0:
	...
	for p in email_classificationDt_list: 
		plot_procedures.plot_ts(ts, out_dir_path + "/user_" + p[0] + ".png", ylim = [-0.01, 1.01], dt_axvline = p[1], compressed = True)
	class_dt_out = []
...
def process_test_tgr():
	...
	for idx, row in df.iterrows():
		dt_cp_list = []
		tp = (row["mac"], row["server"], row["date_start"], row["date_end"]) 
		if tp not in macServerDtstartDtend_emailClassifications: macServerDtstartDtend_emailClassifications[tp] = []
		if row["change_points"] != "\'\'":
			for strdt_js in row["change_points"].split(","):
				if strdt_js != "":
					dt = get_datetime(strdt_js)
					dt_cp_list.append(dt)
		macServerDtstartDtend_emailClassifications[tp].append([row["email"], dt_cp_list])

	macServerDtstartDtend_correctClassification = {}
	for tp in macServerDtstartDtend_emailClassifications:
		macServerDtstartDtend_correctClassification[tp] = majority_voting(tp, macServerDtstartDtend_emailClassifications[tp])
