...
def process():
    ...
    model = inertial_hmm.GaussianHMM(number_of_states, data,
                                     rgzn_modes.INERTIAL)
    ...
    plot_procedures.plot_ts_and_dist(ts, ts_dist, out_file_path,
                                     ylabel=metric,
                                     dist_ylabel="hidden states",
                                     dist_plot_type="scatter",
                                     dist_ylim=[-0.5, number_of_states - 0.5],
                                     dist_yticks=range(number_of_states))
