...
metric_list = ["latency", "loss", "throughput_down", "throughput_up", "jitter",
               "num_bytes_up", "num_bytes_down"]


def get_data_spdatetime(cursor, mac_provider=None):
    metric_datetime_measure = {}
    for metric in metric_list:
        metric_datetime_measure[metric] = {}
    ...
    for doc in cursor:
        ...
        mac = doc["_id"]["mac"]
        if ((mac_provider is not None) and
                ((mac not in mac_provider) or (mac_provider[mac] != "NET"))):
            continue
        dt = datetime_procedures.get_datetime_sp(str(doc["_id"]["date"]))

        # latency
        if (("rtt" in doc) and ("lat" in doc["rtt"]) and
                ("m" in doc["rtt"]["lat"]) and ("s" in doc["rtt"]["lat"]) and
                (float(doc["rtt"]["lat"]["m"]) > 0)):
...
def get_data_spdatetime_per_mac(cursor, mac_provider=None):
    ...
    for doc in cursor:
        ...
        mac = doc["_id"]["mac"]
        if ((mac_provider is not None) and ((mac not in mac_provider) or
                                            (mac_provider[mac] != "NET"))):
            continue
        ...
        if mac not in mac_metric_datetime_measure:
            ...
            for metric in metric_list:
                mac_metric_datetime_measure[mac][metric] = {}
        ...
        if (("thr" in doc) and ("tcp" in doc["thr"]) and
                ...
                ("v" in doc["thr"]["tcp"]["down"])):
            throughput_down = doc["thr"]["tcp"]["down"]["v"]
            mac_metric_datetime_measure[mac]["throughput_down"][dt] = \
                throughput_down
        ...
        if (("thr" in doc) and ("tcp" in doc["thr"]) and
                ...
                ("v" in doc["thr"]["tcp"]["up"])):
            throughput_up = doc["thr"]["tcp"]["up"]["v"]
            mac_metric_datetime_measure[mac]["throughput_up"][dt] = \
                throughput_up
        ...
        if ("bytes" in doc) and ("d" in doc["bytes"]):
            num_bytes_down = doc["bytes"]["d"]
            mac_metric_datetime_measure[mac]["num_bytes_down"][dt] = \
                num_bytes_down
...
def get_data_to_file(cursor, out_path):
    ...
    for doc in cursor:
        ...
        if (("_id" not in doc) or ("mac" not in doc["_id"]) or ("date" not in
                                                                doc["_id"])):
            continue
        ...
        if host not in server_mac_dtMeasuresUf:
            server_mac_dtMeasuresUf[host] = {}
        if mac not in server_mac_dtMeasuresUf[host]:
            server_mac_dtMeasuresUf[host][mac] = []
        server_mac_dtMeasuresUf[host][mac].append([dt, uf, loss, latency,
                                                   bytes_cross_traffic_down,
                                                   bytes_cross_traffic_up])

    if not os.path.exists(out_path):
        os.makedirs(out_path)
    for server in server_mac_dtMeasuresUf:
        if not os.path.exists(out_path + "/" + server):
            os.makedirs(out_path + "/" + server)

        for mac in server_mac_dtMeasuresUf[server]:
            server_mac_dtMeasuresUf[server][mac].sort()

            file = open(out_path + "/" + server + "/" + mac + ".csv", "w")
            file.write("uf,dt,loss,latency,num_bytes_cross_traffic_down,"
                       "num_bytes_cross_traffic_up\n")
