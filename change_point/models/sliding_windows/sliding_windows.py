...
class SlidingWindows(change_point_alg.ChangePointAlg):
    def __init__(self, preprocess_args, win_len, f_dist, thresh):
    ...
    def predict(self, row):
        ...
        while i + 2 * self.win_len - 1 < len(ts.y):
            ...
            if self.f_dist(win1, win2) > self.thresh:
                cps.append(i)
                i += self.win_len
            else:
                i += 1
